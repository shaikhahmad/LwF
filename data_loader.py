		...
		if self.train:
			...
			for i in range(len(self.data)):
				if self.targets[i] in classes:
					curr_img = cv2.resize(self.data[i], (self.img_size, self.img_size))
					curr_img = curr_img.transpose(2,0,1)
					if mean_image is None:
						data.append(curr_img/255.)
					else:
						data.append(curr_img/255. - self.mean_image)

					targets.append(int(self.targets[i]))
			...
			if mean_image is None:
				self.mean_image = np.mean(self.data, axis=0)
